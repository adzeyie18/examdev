...
<?php
	...
	<form action="saveTime.php" method="post" enctype="multipart/form-data">
	<?php 

		$sql =	mysql_query("select * from c9_timetbl where sl=1");
	...
	*EXAM dates should be entered in ascending order</p>
	...
	<div style="font-size: 30px;font-weight: bold;font-style: italic;color: blue">CLASS-IX</div>
	...
	<?php 
		...
		$sql =	mysql_query("select * from c9_timetbl where sl=2");
		$row = mysql_fetch_array($sql);
	...
	<?php 
		...
		$sql =	mysql_query("select * from c9_timetbl where sl=3");
		$row = mysql_fetch_array($sql);
	...
	<?php 
		...
		$sql =	mysql_query("select * from c9_timetbl where sl=4");
		$row = mysql_fetch_array($sql);
	...
	<?php 
		...
		$sql =	mysql_query("select * from c9_timetbl where sl=5");
		$row = mysql_fetch_array($sql);
	...
	<?php 
		...
		$sql =	mysql_query("select * from c9_timetbl where sl=6");
		$row = mysql_fetch_array($sql);
