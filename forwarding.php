			...
			while($rr=mysql_fetch_array($ss,MYSQL_ASSOC))
			{
				...
				else //if($rr['subject'])
				{
					...
					else if($rr['subject']=='Mathematics')
					{
						...
						{
							$pdf->Cell(190,4,$row['mata'].'/___',0,0,'L',0);
						}
						else if(empty($row['mata']) && (!empty($row['matb'])))
						{
							$pdf->Cell(190,4,'___/'.$row['matb'],0,0,'L',0);
						}
					}
					else if($rr['subject']=='Social Sciences')
					{
...
if($exam=="compart" && $class=='12a')
{
	...
	if($class=="12a" && $dist!="KOHIMA")
	{
		...
		while($row=mysql_fetch_array($sql,MYSQL_ASSOC))
		{
			...
			$pdf->SetXY($left,75);
			$pdf->Cell(190,4,'under your custody for the HSSLC Compartmental & Improvement Examination '.$year.' to be held from ',0,0,'L',0);
			$pdf->SetFont('Times','B',12);
			$pdf->SetXY(108,70);
			$pdf->Cell(190,4,$tot,0,0,'L',0);
			$pdf->SetFont('Times','',12);
			...
			$pdf->SetFont('Times','',12);
			$pdf->SetXY($left,80);
			$pdf->Cell(190,4,'the                             as per the routine given enclosed. Kindly issue the confidential packets to the  ',0,0,'L',0);
			$pdf->SetXY($left,85);
			$pdf->Cell(190,4,'concerned Centre Superintendent on the date shown below at the appointed time against each subject:',0,0,'L',0);
			$pdf->SetFont('Times','B',10);
			$pdf->SetXY(40,90);
			$pdf->Cell(190,4,'Timing: Morning('.$Etime.')		Afternoon('.$Vtime.')',0,0,'L',0);
			$pdf->SetXY($left,95);
...
if($exam=="main" && $class=="12a")
{
	if($class=='12a' && $dist=="KOHIMA")
	{
		...
		while($row=mysql_fetch_array($sql,MYSQL_ASSOC))
		{
			...
			if(!empty($row['agv'])){ $tot28=1;} else {$tot28=0;}
			if(!empty($row['atv'])){ $tot29=1;} else {$tot29=0;}
			if(!empty($row['mat'])){ $tot30=1;} else {$tot30=0;}

			$tot=$tot1+$tot2+$tot3+$tot4+$tot5+$tot6+$tot7+$tot8+$tot9+$tot10+$tot11+$tot12+$tot13+$tot14+$tot16+$tot15+$tot17+$tot18+$tot19+$tot20+$tot21+$tot22+$tot23+$tot24+$tot25+$tot26+$tot27+$tot28+$tot29+$tot30;
			$not = mysql_query("select * from notification_tbl where class='12'");
			$get = mysql_fetch_array($not);
			$pdf->SetFont('Times','B',8);
			$pdf->SetXY(10,10);
...
if($exam=="compart" && $class=='12c')
{
	if($class=='12c' && $dist=="KOHIMA")
	{
		...
		while($row=mysql_fetch_array($sql,MYSQL_ASSOC))
		{
			...
			$pdf->SetFont('Times','B',11);
			$pdf->SetXY(36,65);
			$pdf->Multicell(160,4,'Confidential papers of the HSSLC Compartmental & Improvement Examination '.$year,0,'L',false);
			$pdf->SetFont('Times','',12);
			$pdf->SetXY($left,75);
			$pdf->Cell(190,4,'Sir/Madam,',0,0,'L',0);
			$pdf->SetXY(30,80);
			$pdf->Cell(190,4,'Kindly collect the packets on the specific dates for the HSSLC Compartmental & Improvement  ',0,0,'L',0);
			$pdf->SetXY($left,85);
			$pdf->Cell(190,4,'Examination '.$year.',to be held from                             at your centre. The numbers of question papers   ',0,0,'L',0);
	...
	while($row=mysql_fetch_array($sql,MYSQL_ASSOC))
	{
		...
		while($rr=mysql_fetch_array($ss,MYSQL_ASSOC))
		{
			if($rr['subcode']=='mils')
			{
				...
				$pdf->Cell(190,4,$rr['day'],0,0,'L',0);
				$pdf->SetXY(95,$contents+$off);
				$pdf->Cell(190,4,'ii.   Alternative English',0,0,'L',0);
				$pdf->SetXY($valAlign,$contents+$off);
				if(!empty($row['alt']))
				{
					$pdf->Cell(190,4,$row['alt'],0,0,'L',0);
				...
				$off=$off+$inc;
				$pdf->SetXY(95,$contents+$off);
				$pdf->Cell(190,4,'iv. Bengali',0,0,'L',0);
				$pdf->SetXY($valAlign,$contents+$off);
				if(!empty($row['bng']))
				{
					$pdf->Cell(190,4,$row['bng'],0,0,'L',0);
				...
				$off=$off+$inc;
				$pdf->SetXY(95,$contents+$off);
				$pdf->Cell(190,4,'vi.  Lotha',0,0,'L',0);
				$pdf->SetXY($valAlign,$contents+$off);
				if(!empty($row['lta']))
				{
					$pdf->Cell(190,4,$row['lta'],0,0,'L',0);
				...
				$off=$off+$inc;
				$pdf->SetXY(95,$contents+$off);
				$sumi = iconv('UTF-8','windows-1252','vii. Sumi(Sütsah)');
				$pdf->Cell(190,4,$sumi,0,0,'L',0);
				$pdf->SetXY($valAlign,$contents+$off);
				if(!empty($row['smi']))
				{
				...
				$off=$off+$inc;
				$pdf->SetXY(95,$contents+$off);
				$pdf->Cell(190,4,'viii.Tenyidie',0,0,'L',0);
				$pdf->SetXY($valAlign,$contents+$off);
				if(!empty($row['tny']))
				{
					$pdf->Cell(190,4,$row['tny'],0,0,'L',0);
...
if($class=='12s' && $dist=="KOHIMA")
{
	...
	while($row=mysql_fetch_array($sql,MYSQL_ASSOC))
	{
		...
		if(!empty($row['itv'])){ $tot16=1;} else {$tot16=0;}
		if(!empty($row['ttv'])){ $tot17=1;} else {$tot17=0;}
		if(!empty($row['hcv'])){ $tot18=1;} else {$tot18=0;}


		$tot=$tot1+$tot2+$tot3+$tot4+$tot5+$tot6+$tot7+$tot8+$tot9+$tot10+$tot11+$tot12+$tot13+$tot14+$tot15+$tot16+$tot17+$tot18;
		$not = mysql_query("select * from notification_tbl where class='12'");
		$get = mysql_fetch_array($not);
		$pdf->SetFont('Times','B',11);
		$pdf->SetXY(10,10);
